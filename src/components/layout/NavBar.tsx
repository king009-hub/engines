...
const NavBar = () => {
  ...
  const moreCategories = useMemo(() => {
    return mainCategories.slice(6);
  }, [mainCategories]);

  return (
    <>
      {/* Desktop Navigation - only show on large screens (1024px+) */}
      <nav className="bg-[#f6f3eb] sticky top-0 z-50 hidden lg:block border-b border-[#ddd3bf]">
        <div className="container mx-auto px-4">
          <div className="flex items-stretch justify-start gap-0">
          {categoriesLoading ? (
            <div className="flex items-center px-3 py-4 text-[11px] font-normal tracking-wider text-gray-500 uppercase">
            ...
            <>
              {visibleCategories.map(cat => (
                ...
                >
                  <Link
                    ...
                    className="flex items-center justify-center gap-1 px-5 py-4 text-[12px] font-bold tracking-[0.08em] text-foreground hover:bg-[#1b1b1b] hover:text-[#d4af37] transition-colors uppercase whitespace-nowrap border-r border-[#ddd3bf]"
                  >
                        ...
                        return (
                          <div className="absolute top-full left-0 bg-white border border-border shadow-2xl z-50 w-[1100px] -ml-40">
                            <div className="grid grid-cols-5 gap-6 p-6">
                              {ENGINE_PARTS_COLUMNS.map((col, idx) => (
                                <div key={idx} className="flex flex-col gap-4">
                                  <Link
              ...
              {moreCategories.length > 0 && (
                <div className="relative">
                  <DropdownMenu>
                    <DropdownMenuTrigger className="flex items-center justify-center gap-1 px-5 py-4 text-[12px] font-bold tracking-[0.08em] text-foreground hover:bg-[#1b1b1b] hover:text-[#d4af37] transition-colors uppercase whitespace-nowrap outline-none">
                      More <ChevronDown className="h-3 w-3 opacity-60 ml-1 shrink-0" />
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="bg-white border-border min-w-[200px] shadow-xl">
                      {moreCategories.map(cat => (
                        <DropdownMenuItem key={cat.id} asChild>
                          <Link 
                            to={`/products?category=${cat.slug}`}
                            className="w-full px-4 py-3 text-[12px] font-bold uppercase tracking-wider text-foreground hover:text-primary transition-colors cursor-pointer"
                          >
                            {translateDynamic(cat.name)}
                          </Link>
                        </DropdownMenuItem>
