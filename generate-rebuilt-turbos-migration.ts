...
function generateSql() {
  const products = JSON.parse(fs.readFileSync('generated-turbocharger-rebuilt.json', 'utf8'));
  
  let sql = '-- Refresh ONLY Rebuilt Turbochargers (CHRA) with correct grouping and professional names\n';
  sql += 'DELETE FROM public.products WHERE category_id = \'cf30785d-b671-4b20-a03b-779d21861dc3\';\n\n';
  ...
  const filename = `supabase/migrations/${timestamp}_refresh_rebuilt_turbos_only.sql`;
  fs.writeFileSync(filename, sql);
  console.log(`Generated rebuilt-turbos-only SQL migration with ${products.length} unique items to ${filename}.`);
}
